...
public abstract class GAction : BaseMono
{
    ...
    [Tooltip("Indica si esta acción es obligatoria para poder continuar con la siguiente acción del plan en caso de ser interrumpida por ya no cumplirse las precondicones o bien si se ejecutó el evento onGoapBreak. si es true, la acción es obligatoria y se crearía un nuevo plan en caso de interrupción, si es false, se detendrá solo la acción en curso, se ejecutará su posperfom y se continuará con la siguiente acción del plan en caso de haber alguna. Si fuera la última acción del plan, evidentemente se creará un plan nuevo")]
    ...
    [Tooltip("Si es false, se intenta crear un nuevo plan en cada frame y si se consigue uno  de mayor prioridad entonces se finaliza la acción y se pasa a ejecutar el siguiente plan  independientemente de lo establecido en la variable mandatory. .Esto es útil para cambiar de plan en base a los cambios en las condiciones de todos los objetivos y no solo en los de la propia acción, osea, si es true este valor, solo se anulará la acción y creará un nuevo plan si cambian las condiciones de la propia acción o si ya ha terminado. Tenga en cuenta que las acciones no bloqueantes pueden consumir muchos recursos.")]
    ...
    ///finaliza la acción y se pasa a ejecutar el siguiente plan independientemente de lo establecido en la variable mandatory. Esto es útil para cambiar de plan en base a los cambios en las condiciones de todos
