...
public class AI : MonoBehaviour
{
    ...
    public Vector3 positionPrevious_;
    bool backToPos_ = false; ///será true si hay que volver a la posición desde la que fue atacado y bajó la vida por debajo del umbral.
    float health = 100.0f;
    float rotSpeed = 5.0f;
    ...
    void Start()
    {
        ...
        positionPrevious_ = new Vector3(0,0,0);
    ...
    {
        /////////
        positionPrevious_ = transform.position;  ///salvo la posición actual, justo antes de moverme hacia una posición segura.
        backToPos_ = true;
    ///////
    ...
    [Task]
    public void BackToPos() ///vuelvo a la posición previa antes de ser atacado en caso de haber estado huyendo haciendo flee
    {   
        if (backToPos_)
        {
            agent.SetDestination(positionPrevious_);
            backToPos_ = false;
            Task.current.Succeed();
