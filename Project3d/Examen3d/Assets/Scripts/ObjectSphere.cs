...
public class ObjectSphere : MonoBehaviour
{
    ...
    void Update()
    {
      //actualizo mi hud ahora que estoy seguro que todos los start se han ejecutado y es el primer update de este objeto.
        if (isFirstUpdate_)
        {
            ...
            GameEvents.TriggerOnHudUpdate(0,0,0,0,1); //agrego un nuevo objeto a la escena
        }
    ...
    void action(GameObject character, GameObject other)
    {
        if ((other.gameObject == this.gameObject) && (!isErasered)) ///solo si es este objeto
        {
            ...
            GameEvents.TriggerOnHudUpdate(100,0,-50,0,-1); // incrementa 100 de velocidad pero resta 50 puntos
